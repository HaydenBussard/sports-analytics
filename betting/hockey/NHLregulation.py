...
def nhl():
    ...
    while True:
        if i < len(yahoo_home_for):
            if yahoo_home_for[i][0] in yahoo_data[a][0]:
                yahoo_data[a].append(yahoo_home_for[i][4])
                i += 1
                a = 0
                continue
            else:
    ...
    while True:
        if i < len(yahoo_home_against):
            if yahoo_home_against[i][0] in yahoo_data[a][0]:
                yahoo_data[a].append(yahoo_home_against[i][4])
                i += 1
                a = 0
                continue
            else:
    ...
    while True:
        if i < len(yahoo_away_for):
            if yahoo_away_for[i][0] in yahoo_data[a][0]:
                yahoo_data[a].append(yahoo_away_for[i][4])
                i += 1
                a = 0
                continue
            else:
    ...
    while True:
        if i < len(yahoo_away_against):
            if yahoo_away_against[i][0] in yahoo_data[a][0]:
                yahoo_data[a].append(yahoo_away_against[i][4])
                i += 1
                a = 0
                continue
            else:
    ...
    while True:
        if i < len(yahoo_data):
            if yahoo_data[i][4] != '':
                h = h + yahoo_data[i][4]
                print(str(yahoo_data[1][0]) +  ' avg home points against = ' + str(h))
                hf = hf + yahoo_data[i][3]
                print(str(yahoo_data[1][0]) +  ' avg home points for = ' + str(hf))
                i += 1
                continue
            else:
                h = h + yahoo_data[i][2]
    ...
    while True:
        if i < len(yahoo_data):
            if yahoo_data[i][6] != '':
                a = a + yahoo_data[i][6]
                print(str(yahoo_data[1][0]) +  ' avg away points against = ' + str(a))
                af = af + yahoo_data[i][5]
                print(str(yahoo_data[1][0]) +  ' avg away points for = ' + str(af))
                i += 1
                continue
            else:
                a = a + yahoo_data[i][2]
    ...
    while True: # poisson
        if y <= len(matchupAnalysis_NHL):
            ...
            d = (d/10000) * 100
            aTeam = matchupAnalysis_NHL[x][0]
            hTeam = matchupAnalysis_NHL[y][0]
            print('the chance of ' + str(aTeam) + ' and ' + str(hTeam) + ' going to OT is ' + str(d) + '%')
            # w = round(a + h, 2) #
    ...
    nhlData.head()
    nhlData.to_excel('/Users/Hayden/OneDrive/Sports Betting/Hockey/NHL_regulation.xlsx', index=False)
