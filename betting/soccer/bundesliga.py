...
def Bundesliga():
    def rename(teamName):
        ...
        if teamName == 'RB Leipzig':
            teamName = 'RB Leipzig'
            return teamName
        if teamName == 'FSV Mainz' or teamName == 'Mainz':
            teamName = 'Mainz'
            return teamName
        if teamName == 'Hoffenheim':
            teamName = 'Hoffenheim'
            return teamName
        if teamName == 'FC Koln' or teamName == 'FC Köln':
            teamName = 'Cologne'
            return teamName
        if teamName == 'Bochum' or teamName == 'VfL Bochum':
            teamName = 'Bochum'
            return teamName
        ...
        if teamName == 'Leverkusen':
            teamName = 'Leverkusen'
            return teamName
        if teamName == 'Gladbach':
            teamName = 'Gladbach'
            return teamName
        if teamName == 'Freiburg' or teamName == 'SC Freiburg':
            teamName = 'Freiburg'
            return teamName
        if teamName == 'Monchengladbach':
            teamName = 'Monchengladbach'
            return teamName
    ...
    url_soccerstats_Bundesliga = 'https://www.soccerstats.com/homeaway.asp?league=germany'
    html_soccerstats_Bundesliga = requests.get(url_soccerstats_Bundesliga)
    soup_soccerstats_Bundesliga = bs4.BeautifulSoup(
        html_soccerstats_Bundesliga.content, 'html.parser')
    df_soccerstats_Bundesliga = soup_soccerstats_Bundesliga.findAll('tr', attrs={
                                                                    'class': 'odd'})
    stats = []
    a = 0
    for elem in df_soccerstats_Bundesliga:
        r = str(elem.text)
        r = r.split('\n')
        # r = r[1:]
        data = []
    ...
    # print(stats_df)
    url_dk = 'https://sportsbook.draftkings.com/leagues/soccer/germany---1.bundesliga'
    ...
    # df_fte_4.to_excel(
    #   '/Users/Hayden/OneDrive/Sports Betting/Soccer/Bundesliga22.xlsx', index=False
    # )
    bl = []
    bl_indices = df_fte_4.League[df_fte_4.League ==
                                 'BundesligaGermany'].index.tolist()
    ...
    def fte_away_name(chances):
        ...
        while True:
            if away_name[i] not in num_list:
                ...
                break
        if away_name[-1] == ' ':  # Unique to Bundesliga/ "Schalke 04" team
            i -= 1
    ...
    df.to_excel(
        '/Users/Hayden/OneDrive/Sports Betting/Soccer/Bundesliga22.xlsx', index=False, sheet_name='Sorted By FTE Value'
    )
