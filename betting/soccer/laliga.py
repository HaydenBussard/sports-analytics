...
def LaLiga():
    def rename(teamName):
        if teamName == 'Villarreal':
            teamName = 'Villarreal'
            return teamName
        if teamName == 'Real Madrid':
            teamName = 'Real Madrid'
            return teamName
        if teamName == 'Rayo Vallecano':
            teamName = 'Vallecano'
            return teamName
        if teamName == 'FC Barcelona' or 'Barcelona' in teamName:
            teamName = 'Barcelona'
            return teamName
        if teamName == 'Atletico Madrid' or teamName == 'Atlético Madrid':
        ...
        if teamName == 'Real Sociedad':
            ...
            return teamName
        if teamName == 'Espanyol':
            teamName = 'Espanyol'
            return teamName
        if teamName == 'Almeria':
            teamName = 'Almeria'
            return teamName
        if teamName == 'Valladolid':
            teamName = 'Valladolid'
            return teamName
        if teamName == 'Mallorca':
            teamName = 'Mallorca'
            return teamName
        if teamName == 'Athletic Bilbao':
        ...
        if teamName == 'Osasuna':
            teamName = 'Osasuna'
            return teamName
        if teamName == 'Valencia':
            teamName = 'Valencia'
            return teamName
        if teamName == 'Getafe':
            teamName = 'Getafe'
            return teamName
        if teamName == 'Cadiz' or teamName == 'Cádiz':
            teamName = 'Cadiz'
            return teamName
        if teamName == 'Elche':
            teamName = 'Elche'
    ...
    url_soccerstats_LaLiga = 'https://www.soccerstats.com/homeaway.asp?league=spain'
    html_soccerstats_LaLiga = requests.get(url_soccerstats_LaLiga)
    soup_soccerstats_LaLiga = bs4.BeautifulSoup(
        html_soccerstats_LaLiga.content, 'html.parser')
    df_soccerstats_LaLiga = soup_soccerstats_LaLiga.findAll('tr', attrs={
        'class': 'odd'})
    stats = []
    a = 0
    for elem in df_soccerstats_LaLiga:
        r = str(elem.text)
        r = r.split('\n')
        # r = r[1:]
        data = []
    ...
    url_dk = 'https://sportsbook.draftkings.com/leagues/soccer/spain---la-liga'
    ...
    dk = soup_dk.findAll(
        'div', attrs={'class': 'game-props-card17'})  # sportsbook-event-accordion__children-wrapper, sportsbook-outcome-body-wrapper
    print(dk)
    newlist = []
    for span in dk:
    ...
    # df_fte_4.to_excel(
    #   '/Users/Hayden/OneDrive/Sports Betting/Soccer/LaLiga22.xlsx', index=False
    # )
    bl = []
    bl_indices = df_fte_4.League[df_fte_4.League ==
                                 'La LigaSpain'].index.tolist()  # NEEDS TO CHANGE FOR EACH LEAGUE
    ...
    def fte_away_name(chances):
        ...
        while True:
            if away_name[i] not in num_list:
                ...
                break
        if away_name[-1] == ' ':  # Unique to LaLiga/ "Schalke 04" team
            i -= 1
    ...
    df.to_excel(
        '/Users/Hayden/OneDrive/Sports Betting/Soccer/LaLiga22.xlsx', index=False, sheet_name='Sorted By FTE Value'
    )
