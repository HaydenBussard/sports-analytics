...
def FL1():
    def rename(teamName):
        if teamName == 'Lyon':
            teamName = 'Lyon'
            return teamName
        if teamName == 'Marseille':
            teamName = 'Marseille'
            return teamName
        if teamName == 'Paris SG' or teamName == 'PSG':
            teamName = 'Paris St-Germain'
            return teamName
        if teamName == 'Lens':
            teamName = 'Lens'
            return teamName
        if teamName == 'Montpellier':
        ...
        if teamName == 'Lille':
            teamName = 'Lille'
            return teamName
        if teamName == 'Clermont':
            teamName = 'Clermont'
            return teamName
        if teamName == 'Toulouse':
            teamName = 'Toulouse'
            return teamName
        if teamName == 'Auxerre':
            teamName = 'Auxerre'
        ...
        if teamName == 'Nice':
            teamName = 'Nice'
            return teamName
        if teamName == 'Strasbourg':
            teamName = 'Strasbourg'
            return teamName
        if teamName == 'AC Ajaccio':
            teamName = 'Ajaccio'
            return teamName
        if teamName == 'Angers':
            teamName = 'Angers'
    ...
    url_soccerstats_FL1 = 'https://www.soccerstats.com/homeaway.asp?league=france'
    html_soccerstats_FL1 = requests.get(url_soccerstats_FL1)
    soup_soccerstats_FL1 = bs4.BeautifulSoup(
        html_soccerstats_FL1.content, 'html.parser')
    df_soccerstats_FL1 = soup_soccerstats_FL1.findAll('tr', attrs={
        'class': 'odd'})
    stats = []
    a = 0
    for elem in df_soccerstats_FL1:
        r = str(elem.text)
        r = r.split('\n')
        # r = r[1:]
        data = []
    ...
    url_dk = 'https://sportsbook.draftkings.com/leagues/soccer/france---ligue-1'
    ...
    # df_fte_4.to_excel(
    #   '/Users/Hayden/OneDrive/Sports Betting/Soccer/FL122.xlsx', index=False
    # )
    bl = []
    bl_indices = df_fte_4.League[df_fte_4.League ==
                                 'Ligue 1France'].index.tolist()  # NEEDS TO CHANGE FOR EACH LEAGUE
    ...
    def fte_away_name(chances):
        ...
        while True:
            if away_name[i] not in num_list:
                ...
                break
        if away_name[-1] == ' ':  # Unique to FL1/ "Schalke 04" team
            i -= 1
    ...
    print('\n')
    print(
        'FTE PREDICTIONS: THESE SHOULD NOT BE RENAMED:'
    )
    print(stats_df)
    print('\n')
    print('\n')
    ...
    df.to_excel(
        '/Users/Hayden/OneDrive/Sports Betting/Soccer/FL122.xlsx', index=False, sheet_name='Sorted By FTE Value'
    )
