...
def EPL():
    def team_rename(teamName):
        if teamName == 'Leeds Utd' or 'Leeds United' in teamName:
            teamName = 'Leeds'
            return teamName
        if teamName == 'Tottenham':
            teamName = 'Tottenham'
            return teamName
        if teamName == 'Newcastle Utd':
            teamName = 'Newcastle'
            return teamName
        if teamName == 'Fulham':
            teamName = 'Fulham'
        ...
        if teamName == 'Aston Villa':
            ...
            return teamName
        if teamName == 'Nottm Forest':
            teamName = 'Nottingham Forest'
            return teamName
        if teamName == 'Crystal Palace':
        ...
        if teamName == 'Everton':
            teamName = 'Everton'
            return teamName
        if teamName == 'West Ham Utd':
            teamName = 'West Ham'
            return teamName
        else:
            return teamName

    url_soccerstats_EPL = 'https://www.soccerstats.com/homeaway.asp?league=england'
    html_soccerstats_EPL = requests.get(url_soccerstats_EPL)
    soup_soccerstats_EPL = bs4.BeautifulSoup(
        html_soccerstats_EPL.content, 'html.parser')
    df_soccerstats_EPL = soup_soccerstats_EPL.findAll('tr', attrs={
        'class': 'odd'})
    stats = []
    a = 0
    for elem in df_soccerstats_EPL:
        r = str(elem.text)
        r = r.split('\n')
        # r = r[1:]
        data = []
    ...
    # print(stats_df)
    # print(stats_df)
    url_dk = 'https://sportsbook.draftkings.com/leagues/soccer/england---premier-league'
    ...
    # df_fte_4.to_excel(
    #   '/Users/Hayden/OneDrive/Sports Betting/Soccer/EPL22.xlsx', index=False
    # )
    bl = []
    bl_indices = df_fte_4.League[df_fte_4.League ==
                                 'Premier LeagueEngland'].index.tolist()  # NEEDS TO CHANGE FOR EACH LEAGUE
    ...
    def fte_away_name(chances):
        ...
        while True:
            if away_name[i] not in num_list:
                ...
                break
        if away_name[-1] == ' ':  # Unique to EPL/ "Schalke 04" team
            i -= 1
    ...
    df.to_excel(
        '/Users/Hayden/OneDrive/Sports Betting/Soccer/EPL22.xlsx', index=False, sheet_name='Sorted By FTE Value'
    )
