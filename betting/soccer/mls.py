...
def MLS():
    def rename(teamName):
        if teamName == 'Los Angeles FC':
            teamName = 'LAFC'
            return teamName
        if teamName == 'Philadelphia':
            teamName = 'PHI Union'
            return teamName
        if teamName == 'New York City':
            teamName = 'NYCFC'
            return teamName
        if teamName == 'Dallas':
            teamName = 'FC Dallas'
            return teamName
        if teamName == 'Inter Miami':
            teamName = 'MIA CF'
            return teamName
        if teamName == 'Colorado Rapids':
            teamName = 'COL Rapids'
            return teamName
        if teamName == 'Austin':
            teamName = 'Austin FC'
            return teamName
        if teamName == 'Real Salt Lake':
            teamName = 'Real Salt Lake'
            return teamName
        if teamName == 'Charlotte':
            teamName = 'Charlotte FC'
            return teamName
        if teamName == 'Portland':
            teamName = 'POR Timbers'
            return teamName
        if teamName == 'Vancouver':
            teamName = 'VAN Whitecaps FC'
            return teamName
        if teamName == 'CF Montreal':
            teamName = 'MTL Impact'
            return teamName
        if teamName == 'LA Galaxy':
            teamName = 'LA Galaxy'
            return teamName
        if teamName == 'Toronto':
            teamName = 'Toronto FC'
            return teamName
        if teamName == 'Minnesota Utd':
            teamName = 'MIN United'
            return teamName
        if teamName == 'Seattle':
            teamName = 'SEA Sounders'
            return teamName
        if teamName == 'Atlanta Utd':
            teamName = 'ATL United'
            return teamName
        if teamName == 'New England':
            teamName = 'NE Revolution'
            return teamName
        if teamName == 'SJ Earthquakes':
            teamName = 'SJ Earthquakes'
            return teamName
        if teamName == 'Columbus Crew':
            teamName = 'COL Crew'
            return teamName
        if teamName == 'Chicago Fire':
            teamName = 'CHI Fire'
            return teamName
        if teamName == 'Nashville SC':
            teamName = 'Nashville SC'
            return teamName
        if teamName == 'Cincinnati':
            teamName = 'FC Cincinnati'
            return teamName
        if teamName == 'Sporting KC':
            teamName = 'Sporting KC'
            return teamName
        if teamName == 'Houston Dynamo':
            teamName = 'HOU Dynamo'
            return teamName
        if teamName == 'DC United':
            teamName = 'DC United'
            return teamName
        if teamName == 'Orlando City':
            teamName = 'ORL City SC'
            return teamName
        if teamName == 'New York RB':
            teamName = 'New York Red Bulls'
            return teamName
        else:
            return teamName

    url_soccerstats_MLS = 'https://www.soccerstats.com/homeaway.asp?league=usa'
    html_soccerstats_MLS = requests.get(url_soccerstats_MLS)
    soup_soccerstats_MLS = bs4.BeautifulSoup(
        html_soccerstats_MLS.content, 'html.parser')
    df_soccerstats_MLS = soup_soccerstats_MLS.findAll('tr', attrs={
        'class': 'odd'})
    stats = []
    a = 0
    for elem in df_soccerstats_MLS:
        r = str(elem.text)
        r = r.split('\n')
        # r = r[1:]
        data = []
    ...
    url_dk = 'https://sportsbook.draftkings.com/leagues/soccer/usa---mls'
    ...
    # df_fte_4.to_excel(
    #   '/Users/Hayden/OneDrive/Sports Betting/Soccer/MLS22.xlsx', index=False
    # )
    bl = []
    bl_indices = df_fte_4.League[df_fte_4.League ==
                                 'MLSUSA'].index.tolist()  # NEEDS TO CHANGE FOR EACH LEAGUE
    ...
    def fte_away_name(chances):
        ...
        while True:
            if away_name[i] not in num_list:
                ...
                break
        if away_name[-1] == ' ':  # Unique to MLS/ "Schalke 04" team
            i -= 1
    ...
    df = df.sort_values(by=['avg_%_value'], ascending=False)
    ...
    df.to_excel(
        '/Users/Hayden/OneDrive/Sports Betting/Soccer/MLS22.xlsx', index=False, sheet_name='Sorted By Average Value'
    )
